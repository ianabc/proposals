...
RSpec.feature "Locations new", type: :feature do
  ...
  context "Invalid dates" do
    ...
    scenario "start date before end date" do
      ...
      expect(page.body).to have_text("Start date #{@program_year}-06-04 - cannot be greater
                                      than End Date #{@program_year}-05-19".squish)
    end
    ...
    scenario "start date equal to end date" do
      ...
      expect(page.body).to have_text("Start date #{@program_year}-06-04 - cannot be same as
                                      End Date #{@program_year}-06-04".squish)
    end
